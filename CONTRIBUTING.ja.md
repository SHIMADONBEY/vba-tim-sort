...
ローカルで作業する前に Git LFS をインストールし、必要に応じて `git lfs install` を実行してローカル環境で LFS を有効にしてください。クローン後に `test-runner.xlsb` の実体が取得できていない場合は、`git lfs pull` を実行してください。`.gitattributes` をコミットする必要があるのは、`git lfs track` などで追跡ルールを変更した場合のみです。
